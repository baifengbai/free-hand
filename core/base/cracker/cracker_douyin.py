    ...
    def handle_SlideCheck(self, *browser):
        ...
        while (1):
            if(browser==()):
                ...
                movement = SliderChecker_Douyin.getMovement_sliderCheck_douyin_static(self.captchaDstDirPath+'template.png', self.captchaDstDirPath+'target.png', targetWidth)  # 验证码处理器
                ...
                movement = SliderChecker_Douyin.getMovement_sliderCheck_douyin_static(self.captchaDstDirPath+'template.png', self.captchaDstDirPath+'target.png', targetWidth)
                print("滑动的实际距离：", movement)
                # 滑动滑块
                # self.move_sliderTemplate(int(movement))
            ...
            try:
                if(browser==()):
                    # 无指定浏览器
                    sliderTargetSelector = self.browser.find_element_by_xpath(
                        "//img[@id='captcha-verify-image']"
                    )  # 背景
                else:
                    sliderTargetSelector = browser.find_element_by_xpath(
                        "//img[@id='captcha-verify-image']"
                    )  # 背景
