from spider import selenium_douyin_stockA
...
def get_url():
    sql_get = 'SELECT `name`, `code` FROM `tb_stocks_500`;'
    contraler_db = dbOp.Contraler_Database(databaseName='data_usable_database')
    stocks_lis = contraler_db.getAllDataFromDB(sql_get)
    stocks_lis = stocks_lis[200:]
    url_lis = []
    for item in stocks_lis:
        name = Encode.str2urlcode(item[0]+'最新分析')
        code = item[1]
        url = 'https://www.douyin.com/search/' + name + '?publish_time=7&sort_type=0&source=tab_search&type=video'
        url_lis.append(url)
